"""
	 Script That runs and measures latency between the MiddleWare RPI, and the 
"""
...
#import pandas as pd
SAMPLES = 500
f = open("middleware_latency.csv", "w")
f.write('"device", "protocol", "latency_ms\n"')
def measure_modbus(ip, label, slave_id):
    ...
    f.write(label.split("(")[0].strip()+",Modbus TCP,"+str(times)+"\n")
    return times
...
def measure_opcua(ip, label, node_id):
    ...
    print(f"{label}: mean={statistics.mean(times):.2f}ms  "
          f"std={statistics.stdev(times):.2f}ms  "
          f"min={min(times):.2f}ms  max={max(times):.2f}ms", flush=True)
    f.write(label.split("(")[0].strip()+",OPC UA,"+str(times)+"\n")
    return times
